...
def calc():
    ...
    m = abs(int(m1) - int(m2))
    d = abs(int(d1) - int(d2))
    #x = print("Your age is ",y," years and ",m," months ",d," days.")
    label = Label(root, text= f"Your age is {y} years and {m} months {d} days.")
    # this creates x as a new label to the GUI
    label.grid(columnspan=15, row=7, sticky=tk.W, padx=5, pady=5) 
